...
This skill is the detailed operating procedure for the Rolegain LLM call `evidence.chunk-analysis`.
...
- Tool policy: The complete bounded source chunk is in the prompt; external context would break provenance.
- Memory reads: immutable source chunk; source id/version; stable locator
- Memory writes: per-chunk JSON checkpoint; Codex run trace
...
3. Apply the call purpose narrowly: Turn one bounded source chunk into provenance-preserving evidence records. Keep the output focused on
