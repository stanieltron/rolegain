---
name: rolegain-refine-cover-letter
description: Procedure for the Rolegain LLM call application.cover-letter-refine. Invoke only when the runtime explicitly requests this skill for that call.
---

# Cover-letter refinement

This skill is the detailed operating procedure for the Rolegain LLM call `application.cover-letter-refine`.
...
- Fan out: `single`
- Runtime role: `cover-letter-editor`
- Web search mode: `disabled`
- Tool policy: Refinement is bounded to supplied context.
- Memory reads: application context; visible conversation
- Memory writes: revised cover letter; Codex run trace
...
3. Apply the call purpose narrowly: Apply user style guidance without adding unsupported facts. Keep the output focused on
