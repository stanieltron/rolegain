---
name: rolegain-verify-application-schema
description: Procedure for the Rolegain LLM call application.schema-verify. Invoke only when the runtime explicitly requests this skill for that call.
---

# Application schema verification

This skill is the detailed operating procedure for the Rolegain LLM call `application.schema-verify`.
...
- Runtime role: `application-schema-verifier`
- Web search mode: `disabled`
- Tool policy: Verification compares two frozen schemas.
- Memory reads: observed schema; mapped fields
- Memory writes: schema findings; Codex run trace
...
3. Apply the call purpose narrowly: Independently audit one-to-one employer-form mapping. Keep the output focused on
