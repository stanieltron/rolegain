---
name: rolegain-draft-application
description: Procedure for the Rolegain LLM call application.draft. Invoke only when the runtime explicitly requests this skill for that call.
---

# Application draft

This skill is the detailed operating procedure for the Rolegain LLM call `application.draft`.
...
- Fan out: `single`
- Runtime role: `cover-letter-writer`
- Web search mode: `disabled`
- Tool policy: All allowed evidence is supplied in the prompt.
- Memory reads: application context; candidate evidence
- Memory writes: application drafts; Codex run trace
...
3. Apply the call purpose narrowly: Draft grounded cover letters and employer-field answers. Keep the output focused on
...
- deterministic form-field checks
