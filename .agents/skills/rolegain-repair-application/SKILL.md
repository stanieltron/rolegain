---
name: rolegain-repair-application
description: Procedure for the Rolegain LLM call application.repair. Invoke only when the runtime explicitly requests this skill for that call.
---

# Bounded application repair

This skill is the detailed operating procedure for the Rolegain LLM call `application.repair`.
...
- Fan out: `single`
- Runtime role: `application-draft-repairer`
- Web search mode: `disabled`
- Tool policy: Repair cannot acquire new evidence.
- Memory reads: failed drafts; verifier findings
- Memory writes: repaired drafts; Codex run trace
...
3. Apply the call purpose narrowly: Repair only drafts rejected by independent verification. Keep the output focused on
...
> Replacement drafts for failed application ids.

## Verification

- failed-id allow-list
- one repair maximum
