---
name: rolegain-discover-jobs
description: Procedure for the Rolegain LLM call search.web-discovery. Invoke only when the runtime explicitly requests this skill for that call.
---

# Public web search

This skill is the detailed operating procedure for the Rolegain LLM call `search.web-discovery`.
...
- Fan out: `single`
- Runtime role: `public-web-job-researcher`
- Web search mode: `live`
- Tool policy: Discovery needs current public results; page validation is delegated to the browser backend.
- Memory reads: canonical evidence search plan; confirmed preferences; seen URL ledger; prior wave failures
- Memory writes: wave lead list; search queries in trace; Codex run trace
...
3. Apply the call purpose narrowly: Discover live vacancy candidates across evidence-derived search lanes. Keep the output focused on
