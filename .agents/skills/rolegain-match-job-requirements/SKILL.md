---
name: rolegain-match-job-requirements
description: Procedure for the Rolegain LLM call match.requirements. Invoke only when the runtime explicitly requests this skill for that call.
---

# Requirement matching

This skill is the detailed operating procedure for the Rolegain LLM call `match.requirements`.
...
- Runtime role: `job-requirement-assessor`
- Web search mode: `disabled`
- Tool policy: Matching is prompt-only over the persisted vacancy and claim subset.
- Memory reads: validated vacancy; retrieved canonical claims; unknowns and prohibited inferences
- Memory writes: requirement matrix; Codex run trace
...
3. Apply the call purpose narrowly: Map every employer requirement to explicit candidate evidence or an honest gap. Keep the output focused on
