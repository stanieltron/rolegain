---
name: rolegain-map-application-fields
description: Procedure for the Rolegain LLM call application.field-map. Invoke only when the runtime explicitly requests this skill for that call.
---

# Application field mapping

This skill is the detailed operating procedure for the Rolegain LLM call `application.field-map`.
...
- Runtime role: `application-field-interpreter`
- Web search mode: `disabled`
- Tool policy: The complete captured schema is supplied.
- Memory reads: captured employer schema; confirmed candidate facts
- Memory writes: field mappings; Codex run trace
...
3. Apply the call purpose narrowly: Map multilingual employer questions to canonical candidate facts. Keep the output focused on
