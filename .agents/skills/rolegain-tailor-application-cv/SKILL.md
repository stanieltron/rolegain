---
name: rolegain-tailor-application-cv
description: Procedure for the Rolegain LLM call application.cv-tailor. Invoke only when the runtime explicitly requests this skill for that call.
---

# Application CV tailoring

This skill is the detailed operating procedure for the Rolegain LLM call `application.cv-tailor`.
...
- Purpose: Reorder and rewrite an existing CV for one application without adding unsupported facts.
- Fan out: `single`
- Runtime role: `application-cv-tailor`
- Web search mode: `disabled`
- Tool policy: CV tailoring is bounded to the supplied original CV and application context.
- Memory reads: original CV; one grounded application context
- Memory writes: tailored CV; Codex run trace
...
3. Apply the call purpose narrowly: Reorder and rewrite an existing CV for one application without adding unsupported facts. Keep the output focused on
