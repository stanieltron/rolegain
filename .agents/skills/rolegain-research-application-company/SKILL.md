---
name: rolegain-research-application-company
description: Procedure for the Rolegain LLM call application.company-research. Invoke only when the runtime explicitly requests this skill for that call.
---

# Application company research

This skill is the detailed operating procedure for the Rolegain LLM call `application.company-research`.
...
- Purpose: Research the employer behind one application and derive sourced application-tailoring context.
- Fan out: `parallel-per-job`
- Runtime role: `application-company-researcher`
- Web search mode: `live`
- Tool policy: Application-stage company research needs current public facts but no access to candidate files or application forms.
- Memory reads: application-stage vacancy identity; public company sources
- Memory writes: sourced company research; Codex run trace
...
3. Apply the call purpose narrowly: Research the employer behind one application and derive sourced application-tailoring context. Keep the output focused on
