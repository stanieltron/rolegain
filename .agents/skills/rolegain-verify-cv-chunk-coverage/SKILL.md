---
name: rolegain-verify-cv-chunk-coverage
description: Procedure for the Rolegain LLM call evidence.chunk-coverage. Invoke only when the runtime explicitly requests this skill for that call.
---

# Independent chunk coverage verification

This skill is the detailed operating procedure for the Rolegain LLM call `evidence.chunk-coverage`.
...
- Runtime role: `candidate-source-coverage-verifier`
- Web search mode: `disabled`
- Tool policy: Coverage is judged only against the source chunk and proposed extraction supplied in the prompt.
- Memory reads: immutable source chunk; normalized reader extraction
- Memory writes: coverage decision; Codex run trace
...
3. Apply the call purpose narrowly: Detect material omissions and unsupported extraction before evidence leaves the source chunk. Keep the output focused on
