---
name: rolegain-navigate-vacancy-source
description: Procedure for the Rolegain LLM call search.source-navigation. Invoke only when the runtime explicitly requests this skill for that call.
---

# Bounded vacancy-source navigation

This skill is the detailed operating procedure for the Rolegain LLM call `search.source-navigation`.
...
- Pipeline: `02-search`
- Purpose: Reveal additional vacancy links on interactive or infinite-scroll listing pages without applying or authenticating.
- Fan out: `parallel-per-source`
- Runtime role: `vacancy-source-navigator`
- Web search mode: `disabled`
- Tool policy: The model selects one action from an immutable observation; the backend validates and actuates it separately.
- Memory reads: current source-page observation; bounded progress counters
- Memory writes: bounded navigation action; Codex run trace
...
3. Apply the call purpose narrowly: Reveal additional vacancy links on interactive or infinite-scroll listing pages without applying or authenticating. Keep the output focused on
