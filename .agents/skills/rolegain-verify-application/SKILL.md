---
name: rolegain-verify-application
description: Procedure for the Rolegain LLM call application.verify. Invoke only when the runtime explicitly requests this skill for that call.
---

# Independent application verification

This skill is the detailed operating procedure for the Rolegain LLM call `application.verify`.
...
- Fan out: `single`
- Runtime role: `independent-application-verifier`
- Web search mode: `disabled`
- Tool policy: Verification must use the frozen evidence packet.
- Memory reads: application context; generated drafts; code findings
- Memory writes: verification verdicts; Codex run trace
...
3. Apply the call purpose narrowly: Reject unsupported application claims and invalid field answers. Keep the output focused on
...
- application-id completeness
- deterministic findings override model pass
