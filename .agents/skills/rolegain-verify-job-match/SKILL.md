---
name: rolegain-verify-job-match
description: Procedure for the Rolegain LLM call match.verification. Invoke only when the runtime explicitly requests this skill for that call.
---

# Independent match verification

This skill is the detailed operating procedure for the Rolegain LLM call `match.verification`.
...
- Runtime role: `independent-fit-verifier`
- Web search mode: `disabled`
- Tool policy: The verifier must be independent and see exactly the recorded evidence under review.
- Memory reads: vacancy originals; candidate citation ledger; generated matrices; deterministic findings
- Memory writes: verification verdicts; repair instructions; Codex run trace
...
3. Apply the call purpose narrowly: Audit match completeness, classification, and grounding in a fresh context. Keep the output focused on
