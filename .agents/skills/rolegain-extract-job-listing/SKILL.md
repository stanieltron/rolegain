---
name: rolegain-extract-job-listing
description: Procedure for the Rolegain LLM call search.listing-extraction. Invoke only when the runtime explicitly requests this skill for that call.
---

# Listing extraction

This skill is the detailed operating procedure for the Rolegain LLM call `search.listing-extraction`.
...
- Runtime role: `vacancy-list-expander`
- Web search mode: `disabled`
- Tool policy: The call interprets one immutable browser snapshot; it must not fetch a different page.
- Memory reads: frozen listing snapshot; captured link allow-list; candidate intent
- Memory writes: listing vacancy leads; Codex run trace
...
3. Apply the call purpose narrowly: Split a live list or recruitment page into concrete vacancy leads. Keep the output focused on
