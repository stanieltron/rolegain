---
name: rolegain-synthesize-cv-evidence
description: Procedure for the Rolegain LLM call evidence.synthesis. Invoke only when the runtime explicitly requests this skill for that call.
---

# Evidence synthesis and deduplication

This skill is the detailed operating procedure for the Rolegain LLM call `evidence.synthesis`.
...
- Fan out: `single`
- Runtime role: `candidate-intelligence`
- Web search mode: `disabled`
- Tool policy: Synthesis must be a pure reduction over persisted reader outputs.
- Memory reads: current profile; reader outputs; material unknowns
- Memory writes: canonical synthesis JSON; Codex run trace
...
3. Apply the call purpose narrowly: Reduce isolated reader signals into a candidate model and search vocabulary. Keep the output focused on
...
- deterministic claim consolidation
