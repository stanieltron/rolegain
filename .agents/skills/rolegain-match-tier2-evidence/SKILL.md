---
name: rolegain-match-tier2-evidence
description: Procedure for the Rolegain LLM call match.tier2-evidence. Invoke only when the runtime explicitly requests this skill for that call.
---

# Tier 2 unresolved matching

This skill is the detailed operating procedure for the Rolegain LLM call `match.tier2-evidence`.
...
- Runtime role: `tier2-requirement-assessor`
- Web search mode: `disabled`
- Tool policy: Tier 2 receives a selected persisted evidence packet and cannot expand its own context.
- Memory reads: unresolved Tier 1 rows; selected detailed notes; canonical citations
- Memory writes: Tier 2 row replacements; Codex run trace
...
3. Apply the call purpose narrowly: Use detailed evidence only where Tier 1 could not resolve a requirement. Keep the output focused on
...
> Replacement assessments for exactly the unresolved rows, preserving requirement text and provenance.
