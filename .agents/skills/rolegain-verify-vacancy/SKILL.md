---
name: rolegain-verify-vacancy
description: Procedure for the Rolegain LLM call search.vacancy-verification. Invoke only when the runtime explicitly requests this skill for that call.
---

# Vacancy verification

This skill is the detailed operating procedure for the Rolegain LLM call `search.vacancy-verification`.
...
- Runtime role: `vacancy-page-interpreter`
- Web search mode: `disabled`
- Tool policy: Parallel browser workers acquire snapshots; this call only interprets captured evidence.
- Memory reads: frozen vacancy snapshot; structured page data
- Memory writes: vacancy interpretation; field evidence; Codex run trace
...
3. Apply the call purpose narrowly: Classify and extract one concrete vacancy from a frozen page snapshot. Keep the output focused on
