---
name: rolegain-navigate-application
description: Procedure for the Rolegain LLM call application.navigate. Invoke only when the runtime explicitly requests this skill for that call.
---

# Safe application navigation

This skill is the detailed operating procedure for the Rolegain LLM call `application.navigate`.
...
- Runtime role: `application-page-interpreter`
- Web search mode: `disabled`
- Tool policy: The call only selects an action; the backend separately validates and executes it.
- Memory reads: current page observation
...
3. Apply the call purpose narrowly: Reveal an employer application form without submitting or consenting. Keep the output focused on
