---
name: rolegain-refine-application-answer
description: Procedure for the Rolegain LLM call application.answer-refine. Invoke only when the runtime explicitly requests this skill for that call.
---

# Employer-answer refinement

This skill is the detailed operating procedure for the Rolegain LLM call `application.answer-refine`.
...
- Fan out: `single`
- Runtime role: `application-answer-editor`
- Web search mode: `disabled`
- Tool policy: The answer must remain inside the supplied evidence.
- Memory reads: target field; application context
- Memory writes: revised answer; Codex run trace
...
3. Apply the call purpose narrowly: Revise one narrative employer answer using grounded evidence. Keep the output focused on
