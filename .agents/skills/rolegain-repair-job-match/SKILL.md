---
name: rolegain-repair-job-match
description: Procedure for the Rolegain LLM call match.repair. Invoke only when the runtime explicitly requests this skill for that call.
---

# Bounded match repair

This skill is the detailed operating procedure for the Rolegain LLM call `match.repair`.
...
- Runtime role: `job-requirement-repairer`
- Web search mode: `disabled`
- Tool policy: Repair is bounded to recorded defects and cannot seek new evidence.
- Memory reads: failed matrices; verifier findings; vacancy originals; candidate citation ledger
- Memory writes: repaired matrices; Codex run trace
...
3. Apply the call purpose narrowly: Repair verifier-identified defects before one final verification pass. Keep the output focused on
...
> Complete replacement assessments for failed job ids; unaffected jobs are outside the call.
