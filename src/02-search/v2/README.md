...
existing `src/02-search/v1` pipeline remains available as v1 and is not
imported by this directory.

## Public package

| File | Contract |
| --- | --- |
| [`index.ts`](./index.ts) | Complete v2 search entry point |
| [`contracts.ts`](./contracts.ts) | Lead, capture, decision, and audit types |
| [`schemas.ts`](./schemas.ts) | Discovery and frozen-capture classification schemas |
| [`README.md`](./README.md) | Version behavior, benchmark, and tuning |
...
These commands enable all independently versioned v2 components together:
...
Despite the legacy `start-search-v2.ts` compatibility filename, the supported
`*:v2` commands no longer mean search-only.

For an isolated search experiment, set only:
...
For Railway or another deployment using the complete v2 stack, set all three
version variables above or use `npm run start:production:v2` for the server
process. Worker processes must receive the same three environment variables;
the server launcher cannot mutate a separately started worker.
